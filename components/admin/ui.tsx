...
export function Stat({
  ...
  const body = (
    <>
      ...
      <p className="text-[26px] font-semibold tabular-nums text-purple-900 leading-none mt-1.5">
        {value === null
  ...
  const cls =
    ...
    (href ? 'hover:border-purple-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500' : '');
  ...
  return (
    <section className="bg-white border border-gray-200 rounded-lg overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-3">
        <h2 className="text-[13px] font-semibold text-purple-900">{title}</h2>
...
const TONES = {
  ...
  blue: 'bg-purple-50 text-purple-800 border-purple-200',
} as const;
...
export function Td({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <td className={`px-4 py-2.5 text-[13px] text-purple-900 align-middle ${className}`}>{children}</td>;
}
