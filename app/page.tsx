...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-cream-50 overflow-x-clip">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-10">
            {LEAD_OUTCOMES.map((lead, i) => (
              <div key={lead.label}>
                <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr),1.3fr] gap-4 md:gap-10 items-baseline">
                  <p
                    className="font-serif text-navy-900 leading-none"
                  ...
                  <div className="max-w-md">
                    ...
                    <button
                      ...
                      onPointerLeave={() => setOutcome(null)}
                      onFocus={() => setOutcome(s.label)}
                      onBlur={() => setOutcome(null)}
                      onClick={() => setOutcome(outcome === s.label ? null : s.label)}
                      aria-expanded={outcome === s.label}
                      className="text-left group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navy-500 rounded-lg py-1"
                    >
                      <p className="font-serif text-navy-800 text-[24px] leading-none group-hover:text-navy-900 transition-colors">
                        {s.label}
                      </p>
                      <div
                        className="grid transition-all duration-300 ease-out"
                        style={{
                          gridTemplateRows: outcome === s.label ? '1fr' : '0fr',
                          opacity: outcome === s.label ? 1 : 0,
                          marginTop: outcome === s.label ? 8 : 0,
                        }}
                      >
      ...
      {/* ── 09. Opportunity Fund ─────────────────────────────────────────────── */}
      <section
        className="py-16 sm:py-20 px-6 lg:px-10 bg-navy-900"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              ...
              <Link
                ...
                className="group inline-flex items-center gap-2 text-sm font-medium text-white hover:text-amber-300 transition-colors border-b border-navy-700 hover:border-amber-400 pb-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 rounded-sm"
              >
