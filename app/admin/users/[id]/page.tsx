              ...
              <tbody className="divide-y divide-gray-100">
                {mentorships.map((m) => (
                  <tr key={m.id}>
                    ...
                    <Td className="text-gray-700 tabular-nums">{new Date(m.started_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}</Td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <ul className="divide-y divide-gray-100">
            {requests.map((r) => (
              <li key={r.id} className="px-4 py-2.5 flex items-center gap-3 text-[14px]">
                ...
                <span className="ml-auto text-gray-700 tabular-nums text-[13px]">
